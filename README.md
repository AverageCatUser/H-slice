...
**`examplemod.zip`** in this repo is a finished mod — *Bro* — so there's a real
one to look at. Unzip it into `mods/`, add this line to `engineassets/mods.js`,
and it shows up in Freeplay:

```js
HSLICE.load('mods/bro/mod.js');
```

Open its `mod.js` to see what the builder produces: the chart, the metadata, the
characters and the spritesheet frames, all packed into one script.
...
mods/                       your own songs, one folder each
```
